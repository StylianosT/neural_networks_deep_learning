...
for fold, (train_indices, test_indices) in enumerate(k_fold.split(dataset)):
    ...
    # find distribution of train and test sets
    train_counter = defaultdict(int)
    test_counter = defaultdict(int)
    for i in train_sampler.indices:
        train_counter[dataset[i][1]] += 1
    for i in test_sampler.indices:
        test_counter[dataset[i][1]] += 1
    print(f'Train set distribution: {train_counter}')
    print(f'Test set distribution:  {test_counter}')
